...
_VERIFY_SYSTEM_PROMPT = (
    ...
    "Document (ASD). Your job is to decide whether the document "
    "SUBSTANTIVELY addresses that principle anywhere in its body.\n\n"
    "SUBSTANTIVELY means the document either:\n"
    "  (a) commits to a concrete value, target, mechanism, or named Azure "
    "service for the principle — for example: \"RTO of 4 hours\", \"secrets "
    "stored in Azure Key Vault\", \"logs sent to Log Analytics workspace X\", "
    "\"Microsoft Entra ID groups assigned via PIM\"; OR\n"
    "  (b) explicitly defers to a named external policy, standard, or "
    "control framework that covers the principle (e.g. \"see Contoso "
    "Security Standard SEC-014\").\n\n"
    ...
    "  - References to OTHER cloud providers' equivalent services (e.g. "
    "\"AWS Backup\" does NOT address an Azure Backup principle; \"AWS IAM\" "
    "does NOT address an Entra ID identity principle).\n"
    "  - Generic statements like \"we will follow best practices\" without "
    "naming the practice.\n\n"
    "WHEN IN DOUBT, return present=false. A false negative is recoverable "
    "by the reviewer; a false positive silently hides a real gap.\n\n"
